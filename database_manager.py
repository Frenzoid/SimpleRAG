...
class DatabaseManager:
    ...
    def initialize_database(self, chunks, embeddings_model):
        ...
        if os.path.exists(Paths.CHROMADB_PATH) and Configs.CHROMA_LOAD:
            print(f"Loading existing database from {Paths.CHROMADB_PATH}")
            return Chroma(embedding_function=embeddings_model, persist_directory=Paths.CHROMADB_PATH)

        # If the database already exists, but we want to overwrite it, then delete it.
        if os.path.exists(Paths.CHROMADB_PATH):
            print(f"Deleting old database at {Paths.CHROMADB_PATH}")
            shutil.rmtree(Paths.CHROMADB_PATH)
        
        print(f"Creating new database at {Paths.CHROMADB_PATH}")
        return Chroma.from_documents(chunks, embedding=embeddings_model, persist_directory=Paths.CHROMADB_PATH)
